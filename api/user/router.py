...
async def update_realname(auth: schema.Auth, body: schema.ChangeRealname = Depends(), db: Session = Depends(get_db)):
    user_id = await cryptoUtils.get_user_id_by_token(auth.token, db)
...
async def get_photo(auth: schema.Auth, body: schema.PhotoId, db: Session = Depends(get_db)):
    user_id = await cryptoUtils.get_user_id_by_token(auth.token, db)
    return StreamingResponse(io.BytesIO(await crud.get_photo(body.photo_id, user_id, db)), media_type='image/png')
